...
def prepare_data(mat_file, seq_len=64, augment=False, upsampling_scale=10):
    ...
    windowed_data = emg.reshape(n_windows,window_size,seq_len)
    windowed_mean = np.mean(windowed_data,axis=(1,2),keepdims=True)
    windowed_std = np.std(windowed_data,axis=(1,2),keepdims=True)
    windowed_normalized = (windowed_data-windowed_mean) / windowed_std
    emg_standardized = windowed_normalized.reshape(-1,512)
